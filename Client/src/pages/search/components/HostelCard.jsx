...
const AmenityIcon = ({ name }) => {
    switch (name) {
        ...
        case 'Hot Water': return <Zap className="w-4 h-4" />; // Using Zap for Hot Water
...
const HostelCard = ({ hostel }) => {
    ...
    return (
        <div className="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-200 hover:scale-[1.01] cursor-pointer flex flex-col h-full">
            {/* Image Section */}
            <div className="relative aspect-4/3 overflow-hidden bg-gray-100">
                ...
                <div className="absolute top-3 right-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium backdrop-blur-sm shadow-sm ${available > 0
                            ? 'bg-green-100/90 text-green-700'
                            : 'bg-red-100/90 text-red-700'
                        }`}>
                        {available > 0 ? `${available} beds left` : 'Fully Booked'}
                    </span>
                </div>
            </div>
